...
pub fn networking_sockets() -> (
    networking_sockets::NetworkingSockets,
    networking_sockets::SingleNetworkingSockets,
) {
    unsafe {
        let sockets = sys::SteamAPI_SteamNetworkingSockets_v009();
        debug_assert!(!sockets.is_null());
        (
            networking_sockets::NetworkingSockets { sockets },
            networking_sockets::SingleNetworkingSockets::new(),
        )
    }
}
